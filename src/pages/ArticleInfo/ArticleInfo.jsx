...
const ArticleInfo = () => {
  ...
  useEffect(() => {
    setSelectedRating(recentSelectedArticleRating);
  }, [recentSelectedArticleRating]);

  // useEffect(() => {
  //   if (!selectedArticle?._id || !authUser?.userID) return;
  ...
  return (
    <div className="px-5 py-3">
      <div className="d-flex flex-column gap-4">
        ...
        <div
          className="p-4 border shadow-sm rounded-3"
        ...
        <div className="p-4 border shadow-sm rounded-3">
          <div>
            <i className="bi bi-tag text-body-secondary icon-bold"></i> Tags
          </div>
          <div className="d-flex gap-3 mt-2">
            {selectedArticle?.tags?.length > 0 ? (
              selectedArticle.tags.map((tag) => (
                <div key={tag} className="badge rounded-pill border text-dark">
                  #{tag}
                </div>
              ))
            ) : (
        ...
        <div className="p-4 border shadow-sm rounded-3">
          <h5 className="fw-medium m-0">
            <i className="bi bi-hand-thumbs-up me-2"></i>Was this article
            helpful?
          </h5>
          <div className="mt-2">
              ...
              <div className="d-flex align-items-center gap-3">
                ...
                <div className="d-flex gap-1 f-size-16">
                  {[...Array(5)].map((_, index) => {
                    ...
                    const isFilled =
                      starNumber <= (hoveredStar || selectedRating);

                    return (
                      <i
                        key={index}
                        className={`bi ${
                          isFilled ? "bi-star-fill text-warning" : "bi-star"
                        } icon-bold`}
                        style={{ cursor: "pointer" }}
                        onMouseEnter={() => setHoveredStar(starNumber)}
                        onMouseLeave={() => setHoveredStar(null)}
                        onClick={() => {
                          submitArticleRating(starNumber);
          ...
          <div className="d-flex align-items-center justify-content-center rounded-circle bg-primary-subtle text-primary fw-medium">
            {selectedArticle?.author?.profilePic ? (
              <img
                ...
                src={selectedArticle.author.profilePic}
