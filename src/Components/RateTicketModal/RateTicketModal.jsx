...
const RateTicketModal = ({ setShowRateTicketModal }) => {
    const { addTicketRating } = useTicketStore();
        ...
        >
            <div className="d-flex flex-column gap-2 bg-white rounded-3 shadow p-4 m-2" style={{width: '500px'}}>
                ...
                <div className="d-flex justify-content-end gap-2 mt-3 fw-medium">
                    ...
                    <button onClick={()=> {addTicketRating(selectedRating, ticketRatingComment); setShowRateTicketModal(false)}} type="button" className="btn btn-primary fw-medium">Submit Rating</button>
                </div>
            </div>
        </div>
    )
