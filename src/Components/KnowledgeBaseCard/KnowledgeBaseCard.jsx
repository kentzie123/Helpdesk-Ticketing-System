...
const KnowledgeBaseCard = ({ article }) => {
  const { updateArticle, setSelectedArticle, setShowDeleteArticleModal } = useArticleStore();
    ...
    <div className="knowledge-base d-flex justify-content-between border shadow-sm rounded-3 p-4">
      <div className="d-flex flex-column gap-2">
        <div className="d-flex align-items-center gap-3">
          ...
          <div
            className={` rounded-pill f-size-12 fw-medium px-2 d-flex align-items-center ${getCategoryBadgeClass(
              article.category
            )}`}
          >
            {article.category}
        ...
        <div className="d-flex gap-2">
          {article.tags.map((tag, i) => (
            <div
              key={i}
              className="rounded-pill bg-body-secondary px-2 py-1 f-size-12 text-muted"
            >
        ...
        <div className="d-flex gap-3 text-muted f-size-14">
          ...
          <div>
            ...
            {article.ratings.average}
          </div>
          <div>
            <i className="bi bi-clock"></i> Updated{" "}
      ...
      <div className="d-flex align-items-start gap-4">
        <Link to={`/knowledge-base/${article.slug}`} className="mt-1">
          <i className="kb-action-btn bi bi-eye"></i>
        </Link>

        <button
          onClick={() => setSelectedArticle(article)}
          className="btn btn-light f-size-14 fw-medium"
          type="button"
        ...
        {!article.isPublished ? (
          <button
            onClick={() => updateArticle(article.slug, { isPublished: true })}
            type="button"
            className="btn btn-light text-success fw-medium"
          >
            Publish
          </button>
        ) : null}
        <i
          ...
          className="kb-action-btn bi bi-trash text-danger mt-1"
        ></i>
      </div>
    </div>
  );
